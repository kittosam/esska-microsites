...
Run from sites/dublin-2027:  python3 ../../tools/istanbul_site_programme.py"""
import sys, pathlib, re
sys.path.insert(0, str(pathlib.Path(__file__).parent))
import dublin_programme as data
...
def day_block(day, num, label, iso, wk, dnum, mon, full, hidden):
    ...
    # sessions here are not broken into talks, so the talk count is left off
    talks_metric = f'<span>{ICON["mic"]}{talks_txt}</span>' if not talks_txt.startswith("0 ") else ""
    rows=[]
...
f'''        <section class="pday" data-day="{num}"{" hidden" if hidden else ""}>
          <div class="pday-head">
            ...
            <div class="pday-id">
              ...
              <span class="pday-meta"><span>{ICON["clock"]}<b>{span}</b></span><span>{ICON["list"]}{sess}</span>{talks_metric}</span>
            </div>
...
{chr(10).join(f"            <li>{x}</li>" for x in data.PHILOSOPHY)}
          </ul>
        </div>
        <div class="course-col">
      ...
      <div class="prog-controls rv">
        ...
        <div class="pctl-row">
          <div class="pchips" id="progChips" role="group" aria-label="Choose a day">
            <button class="pchip" type="button" data-filter="1" aria-pressed="true">Day one<span class="pchip-date"> &middot; 14 May</span></button>
            <button class="pchip" type="button" data-filter="2" aria-pressed="false">Day two<span class="pchip-date"> &middot; 15 May</span></button>
          </div>
        </div>
      </div>
...
{day_block(data.DAY1,"1","Day one","2027-05-14","Fri","14","May","Friday 14 May 2027",False)}
...
mark='<!-- course-intro -->'
if mark in s:
    s=re.sub(re.escape(mark)+r'.*?'+re.escape('<!-- /course-intro -->'), mark+"\n"+COURSE+"\n      "+'<!-- /course-intro -->', s, flags=re.S)
else:
    anchor='  <section>\n    <div class="wrap">\n      <div class="head rv"><p class="eyebrow">Scientific programme</p>'
    s=s.replace(anchor, '  <section class="tint">\n    <div class="wrap">\n      '+mark+"\n"+COURSE+"\n      "+'<!-- /course-intro -->\n    </div>\n  </section>\n\n'+anchor, 1)
